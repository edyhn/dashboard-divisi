---
id: task-2026-08-26-r4ullg
title: '[TGT-02] Use cases draft/submit/return/approve/lock target'
status: blocked
priority: high
type: other
assignee: ''
created: '2026-08-26T05:04:01.560Z'
updated: '2026-08-26T05:04:01.560Z'
retry_count: 0
source: manual
project: proj-2026-08-26-p06j8t
parent_task: proj-2026-08-26-p06j8t
depends_on:
  - task-2026-08-26-sldbr0
  - task-2026-08-26-8d1ko8
scope: []
context_notes:
  - Dashboard.md
  - >-
    Decisions/2026-08-26-pembagian-peran-owner-ai-coach-reviewer-infra-takeover-dev.md
  - Decisions/2026-08-24-struktur-monorepo-lokasi-ntfs-untuk-dashboard-divisi.md
timeout_minutes: 60
tags:
  - dashboard-divisi
  - mvp
  - backlog-v0.1
review_count: 0
max_retries: 0
retry_delay_minutes: 5
escalation_status: none
---
## Description

Done when: transition hanya melalui command bisnis; reason return wajib. Owner: BE, size L.

## Acceptance Criteria

- [ ] Transisi hanya via command; return wajib reason

## Agent Log

<!-- Agents append their progress updates here -->

### 2026-09-01 — DIGI-11 (agent Beny)

Use case draft/submit/approve/return + lock target selesai di backend (`TargetService`): SUBMITTED tidak bisa disubmit ulang, APPROVED/RETURNED tidak bisa diputuskan lagi (`INVALID_STATE_TRANSITION`). Reopen privileged belum dibuat.
