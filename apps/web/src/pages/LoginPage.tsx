    ...
    <div className="flex min-h-screen items-center justify-center bg-surface p-4">
      <form onSubmit={submit} className="w-full max-w-sm rounded-card-lg border border-line bg-white p-6 shadow-card">
        ...
        <label htmlFor="login-email" className="mt-4 flex flex-col text-sm">
          Email
          <Input id="login-email" value={email} onChange={(e) => setEmail(e.target.value)} className="mt-1" placeholder="nama@divisi.company.id" autoComplete="email" aria-invalid={!!(msg ?? error)} aria-describedby={msg ?? error ? "login-error" : undefined} />
        </label>
