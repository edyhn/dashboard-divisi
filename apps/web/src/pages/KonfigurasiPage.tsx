            ...
            <>
              <div className="mt-4 overflow-x-auto rounded-card-lg border border-line">
                <table className="min-w-[520px] w-full text-left text-sm">
                  ...
                  <tbody className="divide-y divide-line">{rows.slice(0,20).map(o=> <tr key={o.code}><td className="px-4 py-3 font-medium text-navy">{o.code}</td><td className="px-4 py-3 text-slate-600">{o.name}</td><td className="px-4 py-3"><StatusPill status={o.isActive ? 'Aktif' : 'Nonaktif'} /></td></tr>)}</tbody>
                </table>
              </div>
              <p className="mt-2 text-xs text-slate-400 lg:hidden">Geser → untuk lihat kolom</p>
